...
-   `run_rrtm()` allows you to automatically download a file with the
    ...
    -   Results of the model calculations:
        ...
        -   `fluxes`: A tibble containing the fluxes (in Watts per
            ...
            (`sw_up` + `lw_up`), and `total_down (`sw_down`+`lw_down\`).
...
*Δ**T*<sub>2 × CO<sub>2</sub></sub> = 286.9 K − 284.4 K = 2.5 K. You may
remember that in last week’s lab, we calculated the climate sensitivity
with MODTRAN (using constant relative humidity to enable water-vapor
feedback) and got *Δ**T*<sub>2 × CO<sub>2</sub></sub> = 1.2 K for the
